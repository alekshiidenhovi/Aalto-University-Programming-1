...
object SkyPic {
  ...
  def placeStar(skyPic: Pic, star: Star): Pic = {
    val tähdenKuva = circle(12.0 / (star.magnitude + 2), White)
    skyPic.place(tähdenKuva, star.posIn(skyPic))
  }

  def placeConstellation(skyPic: Pic, constellation: Constellation): Pic = {
    val starPic = circle(4, Red)
    var fullConstellation: Pic = skyPic
    def yellowLine(star1: Star, star2: Star) = line(star1.posIn(skyPic), star2.posIn(skyPic), Yellow)
    for (star <- constellation.stars) {
      fullConstellation = fullConstellation.place(starPic, star.posIn(skyPic))
    }
    for (line <- constellation.lines) {
      fullConstellation = fullConstellation.place(yellowLine(line._1, line._2), line._1.posIn(skyPic))
    }
    fullConstellation
  }
  ...
  def create(skyData: StarMap, bgSize: Int) = {
    ...
    val skyWithStars = skyData.stars.foldLeft(darkSky)(placeStar)
    skyData.constellations.foldLeft(skyWithStars)(placeConstellation)
  }
