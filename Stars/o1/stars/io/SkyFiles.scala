...
object SkyFiles {
  ...
  def parseStarInfo(starAsString: String): Star = {
    val eka = starAsString.split(";")
    val koordinaatit = new StarCoords(eka(0).toDouble, eka(1).toDouble)
    val nimi: Option[String] = eka.lift(6)
    val tähti = new Star(eka(3).toInt, koordinaatit, eka(4).toDouble, nimi)
    tähti
  }
