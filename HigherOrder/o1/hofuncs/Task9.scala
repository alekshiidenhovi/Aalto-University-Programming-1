...
object Task9 extends App {
  ...
  def turnElementsIntoResult(numbers: Vector[Int], initialValue: Int, operation: (Int, Int) => Int) = {
    var tulos = initialValue
    for (luku <- numbers) {
      tulos = operation(tulos, luku)
    }
    tulos
  }
  ...
  def positiveCount(existingResult: Int, nextNumber: Int) = {
    if (nextNumber > 0) {
      existingResult + 1
    } else {
      existingResult
  ...
  def productOfNonZeros(existingResult: Int, nextNumber: Int) = {
    if (nextNumber != 0) {
      existingResult * nextNumber
    } else {
      existingResult
    }
  }
