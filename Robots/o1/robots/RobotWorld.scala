...
import scala.jdk.FunctionWrappers.FromJavaLongPredicate
...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth, floorHeight) {
  ...
  def initialElements = for (x <- 0 until this.width; y <- 0 until this.height) yield initialSquare(x, y)
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) = {
    ...
    this.robots = this.robots :+ this.robots.head
    this.robots.head
  }
