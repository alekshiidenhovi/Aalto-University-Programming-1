...
package object o1 extends o1.util.ShortcutAliases {
  ...
  object tili {
    ...
    def talleta(lisää: Int) = this.saldo = this.saldo + max(lisää, 0)
    def nosta(tililtä: Int) = {
      val nykyinen = this.saldo
      this.saldo = max(this.saldo - tililtä, 0)
      min(tililtä, nykyinen)
    }
