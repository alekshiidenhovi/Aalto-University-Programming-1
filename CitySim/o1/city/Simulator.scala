...
class Simulator {
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] = {
    this.allAddresses.filter( n => this.cityMap.apply(n) == demographic )
  }
  ...
  def dissatisfiedResidents: Vector[GridPos] = {
    val noVacants = findDemographic(Vacant)
    def naapurit(location: GridPos) = {
      val vierekkäiset = this.cityMap.neighbors(location, true)
      val matchingNaapurit = vierekkäiset.count( _ == this.cityMap(location) )
      val nonVacantsNaapurit = vierekkäiset.count( _ != Vacant )
      val satisfactionPercent = matchingNaapurit.toDouble / nonVacantsNaapurit
      similarityDesired > 100 * satisfactionPercent
    }
    (allAddresses.filter(naapurit(_)).toBuffer --= noVacants.toBuffer).toVector
  }
  ...
  def moveResidents() = {
    ...
    val randomi = Random.shuffle(dissatisfiedResidents)
    if (vacantAddresses.nonEmpty) {
      for (random <- randomi) {
        val randomVacant = vacantAddresses(Random.nextInt(vacantAddresses.size))
        this.cityMap.update(randomVacant, this.cityMap(random))
        this.cityMap.update(random, Vacant)
        vacantAddresses -= randomVacant
        vacantAddresses += random
    }
  }
  }
  ...
  def residents: Map[Demographic, Vector[GridPos]] = this.cityMap.initialElements.groupBy(  )
