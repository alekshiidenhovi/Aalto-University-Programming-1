...
class GameBoard(width: Int, height: Int, boozeCount: Int) extends Grid[Glass](width, height) {
  ...
  private def placeBoozeAtRandom(howMany: Int) = {
    Random.shuffle(this.allElements.toVector).take(howMany).foreach( _.pourBooze() )
  }
  ...
  def isOutOfWater: Boolean = if (allElements.forall( n => n.isEmpty || n.isBooze )) true else false
  ...
  def drink(target: Glass): Unit = {
    if (target.isBooze) {
      this.boozeGlasses.foreach( n => n.empty() )
    } else {
      target.empty()
    }
  }
